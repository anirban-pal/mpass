...
double inter_energy(cBezier *b1, cBezier *b2, int force_flag)
{
  ...
  if(1) {
    ...
    double F0[] = {  	7.87566119757180826965736741840374e-01,
			-3.19294461400946927298605260148179e+00,
 			2.12551016038061094987307348032346e-08,
 			-8.51267133323685420975834858836606e-02,
			 -2.66988758819828131407803084584884e+00,
			 3.82930119177185326438949157165648e-08,
 			-1.67308657435005642133418746198004e-01,
 			-1.66971753749903162322709704312729e+00,
			 2.55802102546531785854530224345871e-08,
			 -5.35130760837487340353391118696891e-01,
			 -4.55507527295286818169728348948411e-01,
	 		7.63670674145501332702235436399388e-09 };
    
    double F[12], Fnorm = 0, dnorm = 0;
    loop(i,12) {
      //printf(" %.32e\n",integralc[i]+integralc[i+12]);
      //std::cout << integralc[i]+integralc[i+12] << std::endl;
      F[i] = integralc[i]+integralc[i+12];
      Fnorm += F[i]*F[i];
      dnorm += (F[i]-F0[i])*(F[i]-F0[i]);
    }
    
    printf("EPSREL: %le NEVAL %d FRel_error: %.32e Time (ns): %lld\n",EPSREL,nevalf, sqrt(dnorm/Fnorm), dur2.count());
