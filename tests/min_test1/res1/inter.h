...
static int Integrand(const int *ndim, const double xx[],
  ...
  C = sqrt(C);
  ff[0] = V(C)*sqrt(fabs(J1))*sqrt(fabs(J2));	
...
static int I1234(const int *ndim, const double xx[],
  ...
  for(int i=0;i<12;i++)
  {
    F[i] = sqrt(fabs(J1*J2))*cP[i]*f(C);
    F[i+12] = sqrt(fabs(J2/J1))*JP[i]*V(C);
    F[i+24] = sqrt(fabs(J1*J2))*cQ[i]*f(C);
    F[i+36] = sqrt(fabs(J1/J2))*JQ[i]*V(C);
  }
...
double inter_energy(cBezier *b1, cBezier *b2)
{
  ...
  double L12 = (b1->length0 * b2->length0)/(b1->length * b2->length);
  L12 = 1.0;  

  double integralLP[12], errorLP[12], probLP[12];
  if(0) Cuhre(2, 12, dJP, b1, NVEC,
    EPSREL, EPSABS, VERBOSE | LAST,
    MINEVAL, MAXEVAL, KEY,
    STATEFILE, SPIN,
  ...
  if(0) Cuhre(2, 12, dJP, b2, NVEC,
    EPSREL, EPSABS, VERBOSE | LAST,
    MINEVAL, MAXEVAL, KEY,
    STATEFILE, SPIN,
    &nregions, &neval, &fail, integralLQ, errorLQ, probLQ);
  
  double U12 = integral[0]*L12;
  
  b1->cohE += 0.5*U12;
  b2->cohE += 0.5*U12;
  
  loop(i,12) {
    b1->f[i] += -(integralc[i]+integralc[i+12])*L12 + 0.0*(U12/b1->length)*integralLP[i] ;
    b2->f[i] += -(integralc[i+24]+integralc[i+36])*L12 + 0.0*(U12/b2->length)*integralLQ[i];
  }
